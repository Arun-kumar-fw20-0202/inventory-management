...
const SupplierCustomerAutocomplete = ({ onSelectChange={},userData={}, type='supplier', ...props }) => {
   ...
   const { data: supplier, isLoading: fetching, isRefetching } = useSupplierCustomers({ search, limit, type: type, fields: 'name,email,phone,type' })
                     ...
                     <div className="flex gap-2 items-center w-full">
                        ...
                        <div className="flex flex-col w-full">
                           <div className="flex justify-between w-full">
                              ...
                              <Chip size='sm' variant='flat' color={item?.type == 'supplier' ? 'warning' : item?.type == 'customer' ? 'primary' : 'default'} className='capitalize'>{item?.type}</Chip>
                           </div>
